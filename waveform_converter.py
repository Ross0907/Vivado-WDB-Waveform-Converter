...
def application_dir() -> Path:
    """Return the persistent directory used by the running application.

    PyInstaller one-file applications execute their bundled Python modules from
    a temporary extraction directory.  User output must instead live beside
    the executable so it remains available after the application exits.
    """
...
def write_vcd(wf: Waveform, output: Path, timescale: str | None = None) -> int:
    ...
    with output.open("w", encoding="utf-8", newline="\n") as f:
        f.write("$date\n    generated by Vivado WDB Waveform Converter\n$end\n")
...
class ConverterGUI:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title(f"Vivado WDB Waveform Converter {APP_VERSION}")
    ...
    def _poll_background(self) -> None:
        ...
        if not ok:
            ...
            if not (tag == "load" and self._current_load_quiet):
                messagebox.showerror("Vivado WDB Waveform Converter", str(payload), parent=self.root)
    ...
    def _begin_load(self, after: str | None, quiet: bool = False) -> None:
        ...
        if not source_paths:
            if not quiet:
                messagebox.showwarning("Vivado WDB Waveform Converter", "Select a waveform file first.", parent=self.root)
            return
        try:
            jobs = build_batch_jobs(source_paths, cfg_paths)
        except (ConversionError, DecodeError) as exc:
            if not quiet:
                messagebox.showerror("Vivado WDB Waveform Converter", str(exc), parent=self.root)
            return
    ...
    def _remove_highlighted(self) -> None:
        ...
        if not self.selected_names:
            messagebox.showwarning("Vivado WDB Waveform Converter", "At least one signal must remain selected.", parent=self.root)
    ...
    def _start_export(self) -> None:
        if not self.wf:
            return
        if not self.selected_names:
            messagebox.showwarning("Vivado WDB Waveform Converter", "Select at least one signal.", parent=self.root)
            return
        fmt = self._format_key(self.format_var.get())
        if self.wf.source_kind == "vcd" and fmt == "vcd":
            messagebox.showwarning("Vivado WDB Waveform Converter", "VCD to VCD conversion is not required; choose CSV, JSON or Excel.", parent=self.root)
            return
        if len(self.batch_jobs) > 1:
    ...
    def _start_batch_export(self, fmt: str) -> None:
        ...
        def task() -> BatchExportResult:
            ...
            lines = [
                f"Vivado WDB Waveform Converter {APP_VERSION}",
                f"Format: {fmt.upper()}",
                f"Jobs: {len(jobs)}",
...
def cli_main(argv: list[str] | None = None) -> int:
    ...
    if len(jobs) > 1:
        ...
        lines = [f"Vivado WDB Waveform Converter {APP_VERSION}", f"Jobs: {len(jobs)}", f"Failed: {len(failures)}", ""]
