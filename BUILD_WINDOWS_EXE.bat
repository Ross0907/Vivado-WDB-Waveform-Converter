...
cd /d "%~dp0"
title Vivado Waveform Converter 2.0.0 - Windows Build

echo ============================================================
echo Vivado Waveform Converter 2.0.0
...
if errorlevel 1 goto :fail
findstr /C:"2.0.0" _version_check.txt >nul
if errorlevel 1 (
...
>>_version_info.txt echo   ffi=FixedFileInfo(filevers=(2,0,0,0), prodvers=(2,0,0,0), mask=0x3f, flags=0x0, OS=0x40004, fileType=0x1, subtype=0x0, date=(0,0)),
...
>>_version_info.txt echo     StringStruct('FileDescription', 'Vivado Waveform Converter'),
>>_version_info.txt echo     StringStruct('FileVersion', '2.0.0'),
>>_version_info.txt echo     StringStruct('InternalName', 'VivadoWaveformConverter'),
>>_version_info.txt echo     StringStruct('OriginalFilename', 'VivadoWaveformConverter.exe'),
>>_version_info.txt echo     StringStruct('ProductName', 'Vivado Waveform Converter'),
>>_version_info.txt echo     StringStruct('ProductVersion', '2.0.0')
...
if exist VivadoWaveformConverter.spec del /q VivadoWaveformConverter.spec

pyinstaller --noconfirm --clean --onefile --windowed ^
  --name VivadoWaveformConverter ^
...
if not exist "dist\VivadoWaveformConverter.exe" (
...
mkdir "release"
copy /y "dist\VivadoWaveformConverter.exe" "release\VivadoWaveformConverter-v2.0.0.exe" >nul
copy /y "LICENSE" "release\LICENSE.txt" >nul

powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  "$p='release\VivadoWaveformConverter-v2.0.0.exe';" ^
  "$h=(Get-FileHash -Algorithm SHA256 $p).Hash.ToLower();" ^
  "Set-Content -Encoding ascii 'release\VivadoWaveformConverter-v2.0.0.exe.sha256' ($h+'  VivadoWaveformConverter-v2.0.0.exe');" ^
  "Compress-Archive -Force -Path 'release\VivadoWaveformConverter-v2.0.0.exe','release\VivadoWaveformConverter-v2.0.0.exe.sha256','release\LICENSE.txt' -DestinationPath 'VivadoWaveformConverter-v2.0.0-Windows-x64.zip'"
if errorlevel 1 goto :fail
...
del /q "VivadoWaveformConverter.spec" >nul 2>nul
...
echo ============================================================
echo Executable:
echo   %CD%\release\VivadoWaveformConverter-v2.0.0.exe
echo.
echo Release archive:
echo   %CD%\VivadoWaveformConverter-v2.0.0-Windows-x64.zip
echo ============================================================
pause
exit /b 0
